      ...
      base::BindOnce(
          [](base::OnceCallback<void(base::Value)> cb,
            ...
            if (!error.empty()) {
              ...
              // 빈 성공 응답
              std::move(cb).Run(base::Value(base::Value::Dict()));
            }
          },
          std::move(callback)));
}
...
void McpSession::DispatchProtocolMessage(
  ...
  auto parsed = base::JSONReader::ReadAndReturnValueWithError(message);
...
void McpSession::HandleCdpMessage(const base::Value::Dict& message) {
  // CDP 메시지 분류:
...
void McpSession::HandleCdpResponse(int id, const base::Value::Dict& message) {
  // 대기 중인 콜백 찾기
  ...
  if (const base::Value::Dict* error = message.FindDict("error")) {
  ...
  const base::Value::Dict* result = message.FindDict("result");
  if (result) {
...
void McpSession::OnNetworkRequestWillBeSent(const base::Value::Dict& params) {
  ...
  // 요청 URL 추출
  if (const base::Value::Dict* request = params.FindDict("request")) {
...
void McpSession::OnNetworkResponseReceived(const base::Value::Dict& params) {
  ...
  // HTTP 상태 코드 추출
  if (const base::Value::Dict* response = params.FindDict("response")) {
...
void McpSession::OnNetworkLoadingFinished(const base::Value::Dict& params) {
...
void McpSession::OnNetworkLoadingFailed(const base::Value::Dict& params) {
