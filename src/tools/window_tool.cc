...
base::Value::Dict WindowTool::input_schema() const {
...
void WindowTool::Execute(const base::Value::Dict& arguments,
                         McpSession* session,
  ...
  session->SendCdpCommand(
      "Browser.getWindowForTarget", base::Value::Dict(),
            ...
            session->SendCdpCommand(
                ...
                base::BindOnce(
                    [](int wid,
                      ...
                      base::Value::Dict win_info;
                      win_info.Set("windowId", wid);

                      // bounds 정보 추가
                      if (bounds_response.is_dict()) {
                        const base::Value::Dict* bounds_dict =
...
void WindowTool::OnGetWindowBoundsResponse(
  ...
  // CDP 오류 확인
  const base::Value::Dict* err_dict = resp_dict.FindDict("error");
  if (err_dict) {
    const std::string* msg = err_dict->FindString("message");
  ...
  const base::Value::Dict* bounds_source = resp_dict.FindDict("bounds");
  if (!bounds_source) {
