...
' "${DELEGATE_CC}"
    ...
    awk '
      /^}  \/\/ namespace$/ && !func_inserted {
        ...
        print "  const char* mcp_disable = getenv(\"CHROMIUM_MCP\");"
        print "  if (mcp_disable && std::string(mcp_disable) == \"0\") return;"
        ...
        print "  auto config = std::make_unique<mcp::McpServer::Config>();"
        print "  if (mcp_stdio) {"
        print "    config->transport_mode = mcp::McpTransportMode::kStdio;"
        print "  } else {"
        print "    config->transport_mode = mcp::McpTransportMode::kSocket;"
        print "    config->socket_path = socket_path.empty() ? \"/tmp/.chromium-mcp.sock\" : socket_path;"
        print "  }"
        print "  mcp::McpServer::GetInstance()->PrepareConfig(std::move(config));"
        print "}"
        print ""
        func_inserted = 1
      }
